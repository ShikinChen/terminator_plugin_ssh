...
(CC_COL_NAME, CC_COL_IP, CC_COL_USER, CC_COL_PORT, CC_COL_TIME) = range(0, 5)
...
class SSHConnect(plugin.MenuItem):
    ...
    def __init__(self):
        ...
        for part in sections:
            ...
            self.cmd_list.append({'name': name, 'ip': ip, 'user': user,
    ...
    def _save_config(self):
        ...
        for ssh_conf in self.cmd_list:
            ...
            item = {'name': name, 'ip': ip, 'user': user, 'port': port, 'last_time': last_time}
            ssh_key = user + "@" + ip
    ...
    def configure(self, widget, data=None):
        ...
        store = Gtk.ListStore(str, str, str, str, long)
        store.set_sort_column_id(CC_COL_NAME, Gtk.SortType.ASCENDING)

        for ssh_conf in self.cmd_list:
            store.append(
                [ssh_conf['name'], ssh_conf['ip'], ssh_conf['user'], ssh_conf['port'], long(ssh_conf['last_time'])])
    ...
    def update_cmd_list(self, store):
        ...
        i = 0
        while iter:
            (name, ip, user, port, last_time) = store.get(iter,
                                                          CC_COL_NAME,
                                                          CC_COL_IP,
                                                          CC_COL_USER,
                                                          CC_COL_PORT,
    ...
    def _create_command_dialog(self, name_var="", ip_var="", user_var="root", port_var="22", passwd_var=""):
        dialog = Gtk.Dialog(
            _("New SSH Config"),
            None,
            Gtk.DialogFlags.MODAL,
        ...
        table.attach(passwd, 1, 2, 4, 5)

        dialog.vbox.pack_start(table, True, True, 0)
        dialog.show_all()
        return (dialog, name, ip, user, port, passwd)

    def on_new(self, button, data):
        (dialog, name, ip, user, port, passwd) = self._create_command_dialog()
        res = dialog.run()
        item = {}
        if res == Gtk.ResponseType.ACCEPT:
            name_val = name.get_text()
            ip_val = ip.get_text()
            user_val = user.get_text()
            if not name_val:
                name_val = "%s@%s" % (user_val, ip_val)
            item['name'] = name_val
            item['ip'] = ip_val
            item['user'] = user_val
            ...
            if item['ip'] == '' or item['user'] == '' or item['port'] == '':
                ...
                while iter != None:
                    ...
                    iter = store.iter_next(iter)
                if not ssh_exist:
                    store.insert(0, (item['name'], item['ip'], item['user'], item['port'], nowTime()))
